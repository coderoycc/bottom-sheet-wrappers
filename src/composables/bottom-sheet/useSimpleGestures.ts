...
interface UseSimpleGesturesParams {
	contentScrollTop: Ref<number>;
	handleClose: () => void;
	persistent: Ref<boolean>;
}

interface UseSimpleGesturesReturn {
...
/**
 * Simplified gesture composable for simple mode bottom sheet.
 *
 * Handles:
 ...
 */
export function useSimpleGestures(
	params: UseSimpleGesturesParams,
): UseSimpleGesturesReturn {
	const { contentScrollTop, handleClose, persistent } = params;
	...
	const endDrag = (y: number): void => {
		...
		// Stop dragging first — if persistent, the CSS transition will
		// animate the panel back to its original position (bounce effect).
		isDragging.value = false;

		if (persistent.value) {
			// Persistent mode: allow drag animation but prevent closing.
			// Resetting isDragging removes the inline translateY and the
			// CSS transition on the panel smoothly bounces it back.
			return;
		}

		// Normal mode: close on swipe down past threshold
	...
	const handleContentTouchMove = (event: TouchEvent): void => {
		...
		// Simple mode: start drag only when scrolled to top and pulling down
